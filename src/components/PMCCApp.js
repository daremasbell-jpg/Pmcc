      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: 'rgba(0,229,160,0.05)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 12, padding: 14 }}>
          ...
          {data.leapRecommendation ? (<>
            <div style={{ color: '#fff', fontFamily: 'monospace', fontSize: 14, fontWeight: 700, marginBottom: 3 }}>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>
              OI: {data.leapRecommendation.openInterest?.toLocaleString()} · IV: {data.leapRecommendation.impliedVol}
            </div>
          </>) : <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>Options data unavailable</div>}
        </div>
        <div style={{ background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: 12, padding: 14 }}>
          <div style={{ color: '#60a5fa', fontSize: 10, letterSpacing: 1.5, marginBottom: 8, fontWeight: 700 }}>📉 SHORT CALL</div>
          {data.shortCallRecommendation ? (<>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>
              OI: {data.shortCallRecommendation.openInterest?.toLocaleString()} · Spread: {data.shortCallRecommendation.spreadPct}
            </div>
          </>) : <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>Options data unavailable</div>}
        </div>
      </div>
...
export default function PMCCApp() {
  ...
  const analyze = async (forceFresh = false) => {
    ...
    setLoadingStep(`Fetching price & IV data for ${t}…`);
    try {
      // Step 1: Server-side — price, IV rank, trend, score
      const res  = await fetch(`/api/quote?ticker=${t}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Analysis failed');
      setAnalysis(data);

      // Step 2: Client-side — options chain (browser can reach Yahoo Finance, server can't)
      setLoadingStep(`Fetching options chain for ${t}…`);
      try {
        const { fetchOptionsChain, findBestLEAP, findBestShortCall } = await import('../lib/options-client.js');
        const options = await fetchOptionsChain(t);
        if (options?.calls?.length) {
          const leap      = findBestLEAP(options.calls, data.targetLeapStrike);
          const shortCall = findBestShortCall(options.calls, data.targetShortCallStrike);
          const leapCost  = leap?.lastPrice || 0;
          const premium   = shortCall?.premium || 0;
          const enriched  = {
            ...data,
            optionsLiquidity: options.calls.length > 20 ? 'High' : options.calls.length > 5 ? 'Medium' : 'Low',
            leapRecommendation: leap ? {
              strike:        leap.strike,
              estimatedCost: leap.estimatedCost,
              delta:         leap.delta,
              openInterest:  leap.openInterest,
              impliedVol:    leap.impliedVol,
              spreadPct:     leap.spreadPct,
              rationale:     `Deep ITM strike targeting delta ~0.80`,
            } : null,
            shortCallRecommendation: shortCall ? {
              strike:           shortCall.strike,
              estimatedPremium: shortCall.estimatedPremium,
              delta:            shortCall.delta,
              openInterest:     shortCall.openInterest,
              spreadPct:        shortCall.spreadPct,
              yieldOnLeap:      leapCost > 0 ? `${((premium / leapCost) * 100).toFixed(1)}%` : 'N/A',
              rationale:        `~7% OTM targeting delta ~0.20-0.25`,
            } : null,
            breakeven: leapCost > 0 ? {
              priceAtExpiry:   ((leap?.strike || 0) + leapCost - premium).toFixed(2),
              timeToBreakeven: premium > 0 ? `~${Math.ceil(leapCost / premium)} months` : 'N/A',
              detail:          `LEAP $${(leapCost*100).toFixed(0)} ÷ monthly premium $${(premium*100).toFixed(0)}`,
            } : null,
          };
          setAnalysis(enriched);
          cache.current[t] = { data: enriched, ts: Date.now() };
        } else {
          cache.current[t] = { data, ts: Date.now() };
        }
      } catch(optErr) {
        console.warn('Options enrichment failed:', optErr.message);
        cache.current[t] = { data, ts: Date.now() };
      }
