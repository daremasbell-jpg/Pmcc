...
const AlertSettings  = dynamic(() => import('./AlertSettings'), { ssr: false });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0c10', color: '#fff',
      ...
      <div style={{ display: 'flex', margin: '14px 20px 0', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        {[['tracker','📋 My Trades'],['recommend','🔍 Top Picks'],['analyze','📊 Analyze'],['alerts','🔔 Alerts']].map(([key,label]) => (
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        {/* TRACKER */}
        {tab === 'tracker' && <TrackerTab onPositionsChange={setTrackerPositions} />}
        ...
        {/* ALERTS */}
        {tab === 'alerts' && <AlertSettings positions={trackerPositions} />}
